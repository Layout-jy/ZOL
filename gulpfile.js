...
gulp.task('copyfile', function () {
    return gulp.src('ZOLstore/src/font/*')
        .pipe(gulp.dest('ZOLstore/dist/font/'));
});

//3.html文件的压缩
gulp.task('uglifyhtml', function () {
    return gulp.src('ZOLstore/src/*.html')
        .pipe(html()) //应用html包
        .pipe(gulp.dest('ZOLstore/dist/'));
});

//4.压缩css。
gulp.task('uglifycss', function () {
    return gulp.src('ZOLstore/src/css/*.css')
        .pipe(css()) //应用html包
        .pipe(gulp.dest('ZOLstore/dist/css/'));
});

//5.利用sass，生成压缩css。
gulp.task('compilesass', function () {
    return gulp.src('ZOLstore/src/sass/*.scss')
        ...
        .pipe(gulp.dest('ZOLstore/dist/style/'));
});

//6.将es6转成es5,再压缩js
gulp.task('uglifyjs', function () {
    return gulp.src('ZOLstore/src/js/*.js')
        .pipe(babel({ //es6转es5
            presets: ['es2015']
        }))
        .pipe(uglify())
        .pipe(gulp.dest('ZOLstore/dist/js/'));
});
...
gulp.task('runimg', function () {
    return gulp.src('ZOLstore/src/img/*.png')
        .pipe(imagemin())
        .pipe(gulp.dest('ZOLstore/dist/img/'));
});

//8.让原生js支持模块化开发--require
gulp.task("browserify", function () {
    var b = browserify({
        entries: "ZOLstore/dist/js/index.js"//入口文件
    });

    return b.bundle()
        .pipe(source("main.js"))//编译后的入口文件
        .pipe(gulp.dest("ZOLstore/dist/js"));//输出路径
});
...
gulp.task('default',function(){
    //文件路径
    //watch:监听
    watch(['ZOLstore/src/font/*','ZOLstore/src/*.html','ZOLstore/src/sass/*.scss','ZOLstore/src/js/*.js','ZOLstore/src/img/*.png','bundle.js'],
